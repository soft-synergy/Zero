...
export default function OptionCard({
  ...
  const isExclusion = exclusion && !positive
  ...
  return (
    <div
      className={[
        ...
        isExclusion ? styles.optionExclusion : '',
        positive && exclusion ? styles.optionPositive : '',
        !hasImage ? styles.optionNoImage : '',
      ].filter(Boolean).join(' ')}
    ...
    >
      ...
      <span className={`${styles.optionLabel} ${type === 'single' ? styles.optionLabelSingle : ''} ${!hasImage ? styles.optionLabelCentered : ''}`}>{option.label}</span>
      {isExclusion && (
        <div className={styles.exclusionBadge} aria-hidden="true">
