...
function checkPaywall() {
  ...
  const dynamicCopy = /export const COPY:\s*Record<LangCode,\s*Copy>\s*=\s*Object\.fromEntries/.test(src)
  if (copyIdx === -1 && !dynamicCopy) { err('Cannot find COPY object in lib/paywall-copy.ts'); return }
...
function checkReviews() {
  ...
  const dynamicReviews = /export const REVIEWS:\s*Record<LangCode,\s*Review\[]>\s*=\s*Object\.fromEntries/.test(src)
