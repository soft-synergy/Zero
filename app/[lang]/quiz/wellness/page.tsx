...
import styles from '@/app/quiz/wellness/page.module.css'
...
export default function WellnessPage() {
  ...
  const weightLbs = Number(answers[WEIGHT_STEP]) || 200
  const heightCm = Number(answers[HEIGHT_STEP]) || 165
  const bmi = calcBMI(weightLbs, heightCm)
  const bmiCategory = getBMICategory(bmi)

  // Activity from step 3 (routine)
  const routineMap: Record<string, string> = {
    sitting: 'sedentary', feet: 'moderate', active: 'active', home: 'light',
  }
  const lifestyle = t.lifestyle[routineMap[answers[3] as string] ?? 'active'] ?? t.lifestyle['active']

  // Eating habits from step 15
  const habits = Array.isArray(answers[15]) ? (answers[15] as string[]) : []
  let eaterKey: keyof typeof t.eater = 'balanced'
  if (habits.includes('sweets')) eaterKey = 'sweet'
  if (habits.includes('fatty')) eaterKey = 'salty'
  if (habits.includes('late-night')) eaterKey = 'emotional'
  if (habits.length === 0) eaterKey = 'balanced'
  const eaterType = t.eater[eaterKey]

  // Energy motivation from step 6
  const energyMotivationMap: Record<string, keyof typeof t.motivation> = {
    strong: 'high', afternoon: 'moderate', 'pre-meal': 'low', drained: 'low',
  }
  const motivation = t.motivation[energyMotivationMap[answers[6] as string] ?? 'moderate']
